...
<p class="note">
حداقل ۱ و حداکثر ۶ عکس انتخاب کنید.
</p>
...
def lucas_final_create_ad():
    ...
    if request.method == "POST":
        ...
        if not account_name:
            ...
            error = "حداکثر ۶ عکس می‌توانید انتخاب کنید."
        ...
        if not error:
            print("🔥 CREATE-AD POST PASSED VALIDATION")
            ...
            conn.commit()
            print("🔥 AD INSERT COMMITTED:", conn.execute("SELECT last_insert_rowid()").fetchone()[0])
            conn.close()

            return redirect(url_for("lucas_my_ads_fixed"))

    conn.close()

    # ------------------------------------------------------------
    # FORM HTML - LUCAS SHOP NEW DESIGN
    # ------------------------------------------------------------
    html = """
<!doctype html>
<html lang="fa" dir="rtl">
...
body{
    ...
    min-height:100vh;
    background:
        radial-gradient(circle at 50% -10%,rgba(0,119,255,.16),transparent 35%),
        #05070b;
    color:#fff;
    font-family:Tahoma,Arial,sans-serif;
}
...
.logo{
    ...
    justify-content:center;
    border-radius:18px;
    background:linear-gradient(145deg,#087cff,#0055bd);
    box-shadow:0 0 30px rgba(8,124,255,.25);
    font-size:30px;
}
...
textarea:focus{
    border-color:#1684ff;
    box-shadow:0 0 0 2px rgba(22,132,255,.08);
}
...
.choice input:checked + span{
    ...
    border-color:#087cff;
    color:#fff;
    box-shadow:0 0 17px rgba(8,124,255,.20);
}
...
.submit{
    ...
    padding:16px;
    margin-top:18px;
    background:linear-gradient(135deg,#087cff,#0064d8);
    color:#fff;
    font-size:16px;
    font-weight:bold;
    cursor:pointer;
    box-shadow:0 8px 25px rgba(0,105,220,.20);
}
...
<div class="sub">
آگهی فروش اکانت کلش آف کلنز در LUCAS SHOP
</div>
...
<label>وضعیت Supercell ID</label>
...
<div class="section-title">🦸 سطح هیروها</div>

<div class="hint">
با انتخاب TH، فقط لول‌های مجاز همان TH نمایش داده می‌شوند.
</div>
...
<div class="section-title">💰 قیمت اکانت</div>
...
<div class="section-title">📱 آیدی تلگرام</div>
...
<div class="section-title">📝 توضیحات آگهی</div>
...
function makeHeroButtons(containerId,inputId,max){
    ...
    if(max<=0){
        ...
        box.innerHTML='<div class="hero-unavailable">🔒 این هیرو در این TH فعال نیست</div>';
...
# مسیر نهایی ثبت آگهی — هر دو URL به تابع واقعی وصل هستند
app.add_url_rule(
    "/create-ad-final",
...
# اطمینان از اتصال مستقیم /create-ad به تابع واقعی
app.view_functions["create_ad"] = lucas_final_create_ad
