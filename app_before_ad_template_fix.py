...
def success():
    ...
    <body style="margin:0;background:#050b14;color:white;font-family:Tahoma;text-align:center;padding:50px">
    <h1>✅ آگهی با موفقیت ثبت شد</h1>
    <p>آگهی پس از تأیید مدیر در فروشگاه نمایش داده می‌شود.</p>
    <a href="/" style="color:#58a6ff">🏠 بازگشت به فروشگاه</a>
    </body>
    </html>
    """
...
def ad_page(ad_id):
    con=db()
    ad=con.execute("SELECT * FROM ads WHERE id=?", (ad_id,)).fetchone()
    con.close()
    ...
    pics=""
    for img in images:
        pics += '<a href="'+img+'" target="_blank"><img src="'+img+'" style="width:100%;max-height:500px;object-fit:contain;border-radius:16px;margin-bottom:12px;background:#050b14"></a>'
    ...
    <title>LUCAS SHOP</title>
    <style>
    body{margin:0;background:#050b14;color:white;font-family:Tahoma,Arial;padding:12px}
    .box{max-width:800px;margin:auto;background:#0b1626;border:1px solid #203752;border-radius:22px;padding:18px}
    h1{text-align:center}
    .info{background:#07101d;border-radius:14px;padding:14px;margin-top:15px}
    .price{font-size:22px;font-weight:bold;margin-top:15px}
    .back{display:block;text-align:center;color:#8fc4ff;text-decoration:none;margin-top:18px}
    img{display:block}
    </style>
    </head>
    <body>
    ...
    <div class="info">
    <h2>{{ad["account_name"] or "اکانت کلش"}}</h2>
    <p>🏷️ {{ad["account_tag"] or "-"}}</p>
    <p>🏰 {{ad["townhall"] or "-"}}　🔨 {{ad["builder_hall"] or "-"}}</p>
    <p>⭐ لول اکانت: {{ad["level"] or "-"}}</p>
    <p>📝 {{ad["description"] or "بدون توضیحات"}}</p>
    <div class="price">💰 {{ad["price"] or "-"}} تومان</div>
    <p>📱 {{ad["telegram"] or ad["seller"] or "-"}}</p>
    </div>

    <a class="back" href="/">🏠 بازگشت به فروشگاه</a>
    </div>
    </body>
    </html>
    """, ad=ad, pics=pics)
...
def sell():
    ...
    try:
        ...
        con.rollback()
        con.close()
        print("SELL ERROR:", repr(ex))
        return sell_page("خطا در ثبت آگهی: "+str(ex))
    finally:
        try:
            con.close()
        except:
...
def sell_page(error=""):
    ths=""
    for n in range(1,19):
        ths += '<button type="button" class="pick" onclick="choose(this,\'townhall\')" data-v="TH%d">TH %d</button>'%(n,n)

    bhs=""
    for n in range(1,11):
        bhs += '<button type="button" class="pick" onclick="choose(this,\'builder_hall\')" data-v="BH%d">BH %d</button>'%(n,n)

    heroes=[
        ("👑 پادشاه بربر","barbarian_king",110),
        ("🏹 ملکه کماندار","archer_queen",110),
        ("🧙 گرند واردن","grand_warden",85),
        ("🛡️ قهرمان سلطنتی","royal_champion",55)
    ]

    hh=""
    for title,key,mx in heroes:
        hh+='<div class="hero"><b>'+title+'</b><div class="levels">'
        for n in range(1,mx+1):
            hh+='<button type="button" class="lvl" onclick="choose(this,\''+key+'\')" data-v="'+str(n)+'">'+str(n)+'</button>'
        hh+='</div><input type="hidden" name="'+key+'" id="'+key+'"></div>'

    e='<div class="error">'+error+'</div>' if error else ''
...
body{margin:0;background:#050b14;color:#fff;font-family:Tahoma,Arial;padding:12px}
.wrap{max-width:900px;margin:auto}
.card{background:#0b1626;border:1px solid #203752;border-radius:22px;padding:18px}
.logo{text-align:center;font-size:28px;font-weight:900}
.sub{text-align:center;color:#91a5bd;margin:7px 0 20px}
label{display:block;font-weight:bold;margin:15px 0 7px}
input,textarea{width:100%;padding:14px;border-radius:12px;border:1px solid #29435f;background:#07101d;color:#fff;font-size:16px}
textarea{min-height:110px}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.section{border-top:1px solid #203752;margin-top:20px;padding-top:16px}
.choices{display:flex;flex-wrap:wrap;gap:6px}
.pick,.lvl{border:1px solid #315071;background:#11243a;color:white;border-radius:9px;padding:9px 11px}
.pick.active,.lvl.active{background:#147fff;border-color:#147fff}
.hero{background:#081321;border:1px solid #1c3149;border-radius:14px;padding:12px;margin:10px 0}
.hero b{display:block;margin-bottom:9px}
.levels{display:flex;flex-wrap:wrap;gap:4px}
.lvl{min-width:37px;padding:7px}
.submit{width:100%;margin-top:20px;padding:16px;border:0;border-radius:13px;background:#147fff;color:white;font-size:18px;font-weight:bold}
.back{display:block;text-align:center;color:#8fc4ff;text-decoration:none;margin-top:15px}
.error{background:#55202b;border:1px solid #b94458;padding:12px;border-radius:11px;margin-bottom:12px}
...
<div class="wrap">
<div class="card">
<div class="logo">🛒 LUCAS SHOP</div>
<div class="sub">ثبت آگهی فروش اکانت کلش آف کلنز</div>
ERROR

<form method="POST" enctype="multipart/form-data" autocomplete="off">

<label>🎮 نام اکانت</label>
<input type="text" name="account_name" autocomplete="off" required>

<label>🏷️ تگ اکانت</label>
<input type="text" name="account_tag" autocomplete="off" placeholder="#XXXXXXXX" required>

<div class="grid">
<div>
<label>🏰 تاون هال</label>
<div class="choices">THS</div>
<input type="hidden" name="townhall" id="townhall">
</div>

<div>
<label>🔨 بیلدر هال</label>
<div class="choices">BHS</div>
...
<input type="number" name="level" min="1" autocomplete="off">
</div>

<div class="section">
<label>🦸 لول هیروها</label>
HEROES
</div>

<div class="section">
<label>💰 قیمت</label>
<input type="text" name="price" inputmode="numeric" autocomplete="off" required>

<label>📱 آیدی تلگرام فروشنده</label>
<input type="text" name="telegram" autocomplete="off" required>
...
function choose(btn,id){
    document.getElementById(id).value=btn.dataset.v;
    btn.parentElement.querySelectorAll("button").forEach(function(x){
    ...
    html=html.replace("ERROR",e)
    html=html.replace("THS",ths)
    html=html.replace("BHS",bhs)
    html=html.replace("HEROES",hh)
