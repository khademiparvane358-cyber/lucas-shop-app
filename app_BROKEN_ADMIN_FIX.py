...
<style>
body{margin:0;background:#0b0f17;color:#fff;font-family:Tahoma,Arial}
.container{max-width:800px;margin:30px auto;padding:20px}
.card{background:#151b27;border:1px solid #263247;border-radius:16px;padding:18px;margin:15px 0}
.btn{display:inline-block;padding:10px 15px;border-radius:10px;background:#2563eb;color:#fff;text-decoration:none;margin:4px}
.status{padding:8px;border-radius:8px;background:#332b12}
</style>
</head>
<body>
<div class="container">
...
{% if not ads %}
<div class="card">هنوز آگهی‌ای ثبت نکرده‌اید.</div>
{% endif %}

{% for ad in ads %}
<div class="card">
<h2>{{ ad["title"] }}</h2>
<p>💰 قیمت: {{ "{:,}".format(ad["price"] or 0) }} تومان</p>
<p>🎮 لول اکانت: {{ ad["account_level"] or "ثبت نشده" }}</p>
<p>🔖 کد آگهی: {{ ad["listing_code"] or "-" }}</p>

{% if ad["approved"] == 1 %}
<p>🟢 آگهی تأیید شده و در سایت قرار دارد.</p>
{% else %}
<p class="status">🟡 آگهی در انتظار تأیید ادمین است.</p>
{% endif %}

<a class="btn" href="/edit-ad/{{ ad['id'] }}">✏️ ویرایش</a>
<a class="btn" href="/delete-ad/{{ ad['id'] }}"
onclick="return confirm('آیا از حذف این آگهی مطمئن هستید؟')">🗑 حذف</a>
</div>
{% endfor %}

<a class="btn" href="/account">↩️ حساب کاربری</a>
</div>
</body>
</html>
""", ads=ads)
...
.card{background:#151b27;border:1px solid #263247;border-radius:16px;padding:18px;margin:15px 0}
.btn{display:inline-block;padding:10px 15px;border-radius:10px;background:#2563eb;color:#fff;text-decoration:none;margin:4px}
.ok{background:#14532d}.wait{background:#713f12}
</style>
</head>
<body>
<div class="container">
...
<div class="card">
<h2>{{ ad["title"] }}</h2>
<p>👤 فروشنده: {{ ad["username"] or ad["seller_id"] }}</p>
<p>💰 {{ "{:,}".format(ad["price"] or 0) }} تومان</p>
<p>🎮 لول: {{ ad["account_level"] or "-" }}</p>
<p>🔖 {{ ad["listing_code"] or "-" }}</p>

{% if ad["approved"] == 1 %}
<p class="ok">🟢 تأیید شده</p>
{% else %}
<p class="wait">🟡 در انتظار تأیید</p>
<a class="btn" href="/admin/approve/{{ ad['id'] }}">✅ تأیید آگهی</a>
{% endif %}

<a class="btn" href="/delete-ad/{{ ad['id'] }}">🗑 حذف</a>
</div>
{% endfor %}
...
def admin_approve(ad_id):
    ...
    conn = sqlite3.connect("lucas.db")
    conn.execute("UPDATE ads SET approved=1 WHERE id=?", (ad_id,))
    conn.commit()
    conn.close()
...
def edit_ad(ad_id):
    ...
    if not user:
        return render_template_string("""<div dir="rtl" style="font-family:Tahoma;text-align:center;margin:80px auto;max-width:600px"><h1>✅ آگهی ویرایش شد</h1><p>تغییرات شما با موفقیت ثبت شد و آگهی برای تأیید مجدد ادمین ارسال شد.</p><a href="/my-ads">بازگشت به آگهی‌های من</a></div>""")"/login")
