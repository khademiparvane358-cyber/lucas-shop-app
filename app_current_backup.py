...
@app.route("/sell", methods=["GET", "POST"], endpoint="sell_new")
def sell():
...
def sell_page(error=""):
    ...
    heroes=[
        ("👑 پادشاه بربر","barbarian_king",95),
        ("🏹 ملکه کماندار","archer_queen",95),
        ("🧙 گرند واردن","grand_warden",70),
        ("🛡️ قهرمان سلطنتی","royal_champion",45)
    ]
...
.levels{display:flex;flex-wrap:wrap;gap:4px;max-height:180px;overflow:auto}
...
function choose(btn,id){
 ...
 btn.parentElement.querySelectorAll("button").forEach(function(x){x.classList.remove("active")});
