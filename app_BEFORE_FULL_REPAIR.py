...
def admin_approve(ad_id):
    ...
    c.execute(
        "UPDATE ads SET approved=1 WHERE id=?",
        (ad_id,)
    )
...
def edit_ad(ad_id):
    ...
    if request.method=="POST":
        ...
        c.execute("""
            UPDATE ads
            SET title=?,price=?,description=?,approved=0
...
def ad_page(listing_code):
    ...
    ad = con.execute(
        "SELECT * FROM ads WHERE listing_code=? AND approved=1",
        (listing_code,)
    ).fetchone()

    if ad:
        con.execute(
            "UPDATE ads SET views=views+1 WHERE listing_code=?",
            (listing_code,)
        )
        con.commit()
    ...
    for key in ["image","image2","image3","image4","image5","image6"]:
...
<div class="value">{{ val("town_hall",val("townhall",val("th"))) }}</div>
</div>

<div class="row">
<div class="label">بیلدر هال</div>
<div class="value">{{ val("builder_hall",val("builderhall",val("bh"))) }}</div>
</div>
