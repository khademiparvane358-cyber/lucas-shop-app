...
# ============================================================
# LUCAS SHOP - CLEAN FINAL ADS SYSTEM
# ============================================================

import sqlite3 as _LS_SQL
import os as _LS_OS


_LS_DB=_LS_OS.path.join(
    _LS_OS.path.dirname(__file__),
    "lucas_shop.db"
)


def _ls_db():
    c=_LS_SQL.connect(_LS_DB)
    c.row_factory=_LS_SQL.Row
    return c
...
def _ls_repair_db():
    ...
    cols=[
        x["name"]
        for x in c.execute("PRAGMA table_info(ads)").fetchall()
    ]

    if "listing_code" not in cols:
        c.execute("ALTER TABLE ads ADD COLUMN listing_code TEXT")

    if "ad_number" not in cols:
        c.execute("ALTER TABLE ads ADD COLUMN ad_number INTEGER")
    ...
    for r in rows:
        c.execute(
            "UPDATE ads SET listing_code=? WHERE id=?",
            (
                "LS-%04d"%int(r["ad_number"]),
                r["id"]
            )
        )

    # آگهی جدید خودش شماره می‌گیرد
    c.execute("""
    CREATE TRIGGER IF NOT EXISTS lucas_ads_auto_number
    AFTER INSERT ON ads
    WHEN NEW.ad_number IS NULL
         OR NEW.ad_number=0
         OR NEW.listing_code IS NULL
         OR NEW.listing_code=''
    BEGIN
...
def _ls_images(ad):
    ...
    # هر سه مدل نام ستون را پشتیبانی می‌کنیم
    names=[
        "image1",
        "image2",
        "image3",
...
def _ls_find_ad(code):
    ...
    # شماره نمایشی مثل 1 یا #1
    if not ad:

        x=code.replace("#","").strip()
...
""",404
    ...
    images=_ls_images(ad)

    number=_ls_val(ad,"ad_number","id")
    ...
    # --------------------------------------------------------
    # گالری
    # همه عکس‌ها فقط داخل قاب اصلی قالب
    # --------------------------------------------------------
...
box-shadow:
0 0 18px rgba(0,120,255,.25),
inset 0 0 35px rgba(0,90,255,.12);
...
box-shadow:
inset 0 0 30px rgba(0,80,255,.18);
...
<!-- سمت چپ: تمام عکس‌های آگهی فقط داخل این قاب -->
...
@app.before_request
def _lucas_clean_ads_intercept():
...
import sqlite3 as _LS_SQL
from flask import request as _LS_REQUEST
from flask import redirect as _LS_REDIRECT
from flask import render_template_string as _LS_RENDER


_LS_DB = str(Path(__file__).resolve().parent / "lucas_shop.db")
...
# ------------------------------------------------------------
# تعمیر دیتابیس آگهی‌ها
# ------------------------------------------------------------

def _LS_REPAIR_DATABASE():
    ...
    for r in rows:
        if r["ad_number"]:
            code = "LS-%04d" % int(r["ad_number"])
        else:
            code = "LS-%04d" % int(r["id"])
    ...
    # --------------------------------------------------------
    # همگام‌سازی ستون‌های قدیمی و جدید
    # --------------------------------------------------------
    ...
    def has(*names):
        return all(x in cols for x in names)
    ...
    if has("townhall", "th"):
        c.execute("""
            ...
            SET townhall=th
            WHERE
                (townhall IS NULL OR townhall='' OR townhall=0)
                AND th IS NOT NULL
                AND th!=''
                AND th!=0
        """)

    if has("builderhall", "bh"):
        c.execute("""
            UPDATE ads
            SET builderhall=bh
            WHERE
                (builderhall IS NULL OR builderhall='' OR builderhall=0)
                AND bh IS NOT NULL
                AND bh!=''
                AND bh!=0
        """)
    ...
    # Builder Hall جدید
    if has("builderhall", "builder_hall"):
        c.execute("""
            UPDATE ads
            SET builderhall=builder_hall
            WHERE
                (builderhall IS NULL OR builderhall='' OR builderhall=0)
                AND builder_hall IS NOT NULL
                AND builder_hall!=''
                AND builder_hall!=0
        """)
    ...
    hero_pairs = [
        ...
        ("archer_queen", "queen"),
        ("grand_warden", "warden"),
        ("royal_champion", "royal")
    ]

    for new_col, old_col in hero_pairs:

        if has(new_col, old_col):

            c.execute(f"""
                UPDATE ads
                SET {new_col}={old_col}
                WHERE
                    ({new_col} IS NULL OR {new_col}='')
                    AND {old_col} IS NOT NULL
                    AND {old_col}!=''
            """)

    # --------------------------------------------------------
    # تریگر برای آگهی‌های جدید
    # --------------------------------------------------------

    c.execute("""
        CREATE TRIGGER IF NOT EXISTS
        lucas_ads_auto_number_final
...
# ------------------------------------------------------------
# مقدار گرفتن از چند ستون احتمالی
# ------------------------------------------------------------

def _LS_FIELD(ad, *names):
...
"""), 404
    ...
    number = _LS_FIELD(ad, "ad_number", "id")
    ...
    if str(supercell).lower() in [
        ...
        supercell_class = "green"

    elif str(supercell).lower() in [
        "0",
        "not_connected",
        "متصل نیست",
        "قطع"
        ...
        supercell_text = "🔴 متصل نیست"

        supercell_class = "red"

    else:

        supercell_text = supercell
    ...
    if gems != "—":
        ...
        name_change_text = name_change


    # --------------------------------------------------------
    # گالری داخل پنل بزرگ سمت چپ قالب
    # --------------------------------------------------------
    ...
    if images:
        ...
        for i, img in enumerate(images):
            ...
            gallery_html += f"""
                <button
                    class="thumb {active}"
                    onclick="showLucasImage('{img}',this)"
                    type="button"
                >
                    <img src="/uploads/{img}">
                </button>
        ...
        gallery_html = """
        <div class="gallery no-photo">
            <div class="game-icon">🎮</div>
            <div>تصویری برای این آگهی ثبت نشده</div>
        </div>
        """
...
.ad-template{{
    ...
    border:3px solid #218cff;

    border-radius:18px;

    box-shadow:
        0 0 8px #1687ff,
        0 0 25px rgba(0,130,255,.45),
        inset 0 0 30px rgba(0,90,255,.15);
...
.ad-template:before{{
    ...
    position:absolute;
    inset:8px;
    border:1px solid rgba(70,170,255,.65);
    border-radius:12px;
    pointer-events:none;
}}
...
.left-area{{
    ...
    box-shadow:
        inset 0 0 35px rgba(0,70,255,.18);
}}
...
.thumb.active{{
    border-color:#2995ff;
    box-shadow:0 0 10px rgba(20,130,255,.8);
}}
...
.title-box{{
    ...
    box-shadow:
        inset 0 0 15px rgba(0,100,255,.1);
}}
...
.description{{
    ...
    border:1px solid #176fbd;

    border-radius:9px;

    background:rgba(1,7,18,.92);
...
<div class="stat-value {supercell_class}">
{supercell_text}
</div>
...
{("" if description == "—" else "<br><b>توضیحات:</b> " + description)}
</div>
...
function showLucasImage(src,button){{

    var main = document.getElementById("mainAdImage");

    if(main){{
        main.src = "/uploads/" + src;
    }}
...
<a class="nav-btn primary" href="/create-ad">
📢 ثبت آگهی
</a>
...
# ============================================================
# جایگزینی Route های خراب قبلی
# ============================================================

def _LS_REMOVE_ROUTE(rule_text):
    ...
    for rule in list(app.url_map.iter_rules()):
        ...
        try:
            ...
            endpoint_rules = app.url_map._rules_by_endpoint.get(
                rule.endpoint,
                []
            )
...
_LS_REMOVE_ROUTE("/explore")


for _rule in list(app.url_map.iter_rules()):

    # فقط /ad/<code>
    # route های edit/delete/approve دست نخورند

    if (
        _rule.rule.startswith("/ad/<")
        and _rule.rule.count("<") == 1
        and _rule.rule.endswith(">")
    ):
        ...
        try:
            ...
            _x = app.url_map._rules_by_endpoint.get(
                _rule.endpoint,
                []
            )
...
app.add_url_rule(
    "/ad/",
    "lucas_final_ad_empty",
    lambda: _LS_REDIRECT("/explore"),
    methods=["GET"]
)

print("======================================")
print("✅ LUCAS FINAL ADS REPAIR ACTIVE")
print("✅ OLD + NEW ADS: ON")
print("✅ #1 #2 #3 CODES: ON")
print("✅ /ad/LS-0001: ON")
print("✅ /ad/#1: ON")
print("✅ 1-3 IMAGES INSIDE TEMPLATE: ON")
print("✅ TH/BH/HERO VALUES: ON")
print("✅ PLAYER TAG HIDDEN: ON")
print("✅ TELEGRAM HIDDEN: ON")
print("======================================")
...
# ===== LUCAS_REAL_FINAL_REPAIR_V2 =====
# ============================================================
# LUCAS SHOP - FINAL ALL ADS REPAIR
# ============================================================

import sqlite3 as _FINAL_SQLITE
import html as _FINAL_HTML
from flask import redirect as _FINAL_REDIRECT

_FINAL_DB = "lucas_shop.db"


def _final_db():
    c = _FINAL_SQLITE.connect(_FINAL_DB)
    c.row_factory = _FINAL_SQLITE.Row
    return c
...
def _final_repair_database():
    ...
    # آگهی‌های جدید هم خودکار شماره می‌گیرند
    c.execute("""
        CREATE TRIGGER IF NOT EXISTS lucas_ads_auto_number_v2
        AFTER INSERT ON ads
        WHEN NEW.ad_number IS NULL
             OR NEW.ad_number=0
             OR NEW.listing_code IS NULL
...
_final_repair_database()


# ------------------------------------------------------------
# مقدار واقعی از چند نام احتمالی ستون
# صفر یعنی مقدار وارد نشده
# ------------------------------------------------------------

def _final_value(ad, *names):
    ...
    for name in names:
        ...
        # برای TH/BH/Hero مقدار صفر را خالی حساب می‌کنیم
        if text == "0":
            continue
...
def _final_ad_page(code):

    ad = _final_find_ad(code)
...
""", 404
    ...
    images = _final_images(ad)

    number = _final_value(ad, "ad_number", "id")
    account = _final_value(ad, "account_name", "title")
    ...
    if supercell in (
        ...
        supercell_show = "🔴 متصل نیست"

    else:
        supercell_show = supercell

    # --------------------------------------------------------
    # تغییر نام
    # --------------------------------------------------------

    if gems != "ثبت نشده" and name_change != "ثبت نشده":
        name_change_show = (
            _FINAL_HTML.escape(name_change)
            + " — "
            + _FINAL_HTML.escape(gems)
            + " 💎"
        )
    else:
        name_change_show = _FINAL_HTML.escape(name_change)
    ...
    if images:
        ...
        gallery = """
        <div class="picture-frame no-picture">
            🎮
...
.template{
    ...
    border:2px solid #1674ff;

    border-radius:25px;

    box-shadow:
        0 0 15px rgba(0,110,255,.55),
        inset 0 0 35px rgba(0,90,255,.13);
...
.left-panel{
    ...
    box-shadow:
        inset 0 0 30px rgba(0,100,255,.14),
...
.picture-frame{
    ...
    box-shadow:
        inset 0 0 25px rgba(0,90,255,.18);
}
...
.field{
    ...
    box-shadow:
        inset 0 0 15px rgba(0,80,255,.10);
}
...
function showPic(src){
    ...
    var img =
        document.getElementById("mainPicture");
...
""" % (
        gallery,
        _FINAL_HTML.escape(number),
        _FINAL_HTML.escape(th),
        _FINAL_HTML.escape(bh),
        _FINAL_HTML.escape(king),
        _FINAL_HTML.escape(queen),
        _FINAL_HTML.escape(warden),
        _FINAL_HTML.escape(royal),
        name_change_show,
        _FINAL_HTML.escape(supercell_show),
        _FINAL_HTML.escape(price),
        (
            '<div class="description">'
            + _FINAL_HTML.escape(description)
            + '</div>'
        )
        if description != "ثبت نشده"
        else ""
    )
...
def _final_explore_page():
    ...
    for ad in ads:
        ...
        """ % (
            image_html,
            _FINAL_HTML.escape(number),
            _FINAL_HTML.escape(account),
            _FINAL_HTML.escape(th),
            _FINAL_HTML.escape(bh),
            _FINAL_HTML.escape(price),
            _FINAL_HTML.escape(code)
        ))
...
.hero{
    ...
    background:
        ...
        linear-gradient(145deg,#091426,#050b16);
}
...
# ------------------------------------------------------------
# جایگزین کردن توابع اصلی
# ------------------------------------------------------------

_LUCAS_OLD_AD_PAGE = globals().get("_LUCAS_AD_PAGE")
_LUCAS_OLD_EXPLORE_PAGE = globals().get("_LUCAS_EXPLORE_PAGE")


def _LUCAS_AD_PAGE(code):
    return _final_ad_page(code)


def _LUCAS_EXPLORE_PAGE():
    return _final_explore_page()
...
try:
    ...
    if "explore" not in app.view_functions:

        app.add_url_rule(
            "/explore",
            endpoint="explore",
            view_func=_final_explore_page
        )
...
# ------------------------------------------------------------
# /ad/ خالی دیگر 500 نمی‌دهد
# ------------------------------------------------------------

try:

    if "lucas_empty_ad" not in app.view_functions:

        app.add_url_rule(
            "/ad/",
            endpoint="lucas_empty_ad",
            view_func=lambda: _FINAL_REDIRECT("/explore")
        )
...
print("======================================")
print("✅ LUCAS FINAL ADS REPAIR V2 ACTIVE")
print("✅ OLD + NEW ADS")
print("✅ #1 #2 #3...")
print("✅ LS-0001 / LS-0002 ...")
print("✅ /ad/ NUMBER LOOKUP")
print("✅ 1-3 IMAGES INSIDE FRAME")
print("✅ TH / BH / HERO REAL VALUES")
print("✅ PLAYER TAG HIDDEN")
print("✅ TELEGRAM HIDDEN")
print("✅ ACCOUNT NAME SHOWN")
print("✅ EXPLORE ENDPOINT FIXED")
print("✅ HOME BUILDERROR FIXED")
print("======================================")
...
# ============================================================
# LUCAS SHOP - EXACT TEMPLATE STYLE V4
# ============================================================

import sqlite3
from flask import redirect

def LUCAS4_GET(ad, *fields, default="ثبت نشده"):
    for f in fields:
        try:
            v = ad[f]
        except Exception:
        ...
        if v is not None:
            v = str(v).strip()
            if v and v not in ("0","None","null","NULL"):
                return v
    return default

def LUCAS4_NUM(ad, *fields):
    for f in fields:
        try:
            v = int(ad[f])
            if v > 0:
...
def LUCAS4_FIND(code):
    ...
    try:
        if c.upper().startswith("LS-"):
            ad = db.execute(
                "SELECT * FROM ads WHERE listing_code=? LIMIT 1",
                (c.upper(),)
            ).fetchone()
        ...
        if ad is None:
            ...
            if n.isdigit():
                ad = db.execute(
                    "SELECT * FROM ads WHERE ad_number=? LIMIT 1",
                    (int(n),)
                ).fetchone()

            if ad is None and n.isdigit():
                ad = db.execute(
...
def LUCAS4_IMAGES(ad):
    arr = []
    for f in ("image1","image2","image3","image"):
        try:
            x = ad[f]
        except Exception:
            x = None
        ...
        if x:
            ...
            if x and x not in arr:
                if x.startswith("http://") or x.startswith("https://"):
                    ...
                    arr.append("/uploads/" + x)

    return arr[:3]


def LUCAS4_AD_PAGE(code):
    ...
    th = LUCAS4_NUM(ad,"townhall","th")
    bh = LUCAS4_NUM(ad,"builderhall","builder_hall","bh")

    king = LUCAS4_NUM(ad,"barbarian_king","king")
    queen = LUCAS4_NUM(ad,"archer_queen","queen")
    warden = LUCAS4_NUM(ad,"grand_warden","warden")
    royal = LUCAS4_NUM(ad,"royal_champion","royal")

    change = LUCAS4_GET(ad,"name_change",default="ثبت نشده")
    gems = LUCAS4_GET(ad,"name_change_gems","gems",default="ثبت نشده")

    sc = LUCAS4_GET(ad,"supercell_id","supercell",default="ثبت نشده")
    level = LUCAS4_GET(ad,"account_level","level",default="ثبت نشده")
    price = LUCAS4_GET(ad,"price",default="ثبت نشده")

    imgs = LUCAS4_IMAGES(ad)

    if not imgs:
        imgs = ["/static/placeholder.png"]
    ...
    for i,x in enumerate(imgs):
        thumbs += (
            '<button class="thumb%s" onclick="showImg(%r,this)">'
            '<img src="%s"></button>'
        ) % (
            " active" if i == 0 else "",
            x,
    ...
    if cv in ("1","true","yes","بله","دارد"):
        change = "دارد"
    elif cv in ("0","false","no","خیر","ندارد"):
        change = "ندارد"

    sv = str(sc).lower()

    if sv in ("1","true","yes","بله","دارد"):
        sc = "متصل"
    elif sv in ("0","false","no","خیر","ندارد"):
        sc = "متصل نیست"
...
box-shadow:
inset 0 0 30px rgba(0,80,255,.25),
0 0 12px rgba(0,120,255,.35);
...
box-shadow:
inset 0 0 12px rgba(0,80,255,.10);
}
...
# پیدا کردن route واقعی قبلی و جایگزین کردن فقط تابع نمایش
_lucas4_endpoint = None

for rule in app.url_map.iter_rules():
    if rule.rule == "/ad/<code>":
        _lucas4_endpoint = rule.endpoint
        break

if _lucas4_endpoint:
    app.view_functions[_lucas4_endpoint] = LUCAS4_AD_PAGE

print("======================================")
print("✅ LUCAS SHOP EXACT TEMPLATE V4")
print("✅ ORIGINAL BLUE FRAME STYLE")
print("✅ LARGE IMAGE FRAME")
print("✅ 1-3 IMAGES INSIDE SAME FRAME")
print("✅ ACCOUNT NAME")
print("✅ AD NUMBER #1 #2 #3")
print("✅ TH / BH")
print("✅ ALL HEROES")
print("✅ NAME CHANGE + GEMS")
print("✅ SUPERCELL ID")
print("✅ ACCOUNT LEVEL")
print("✅ PRICE")
print("✅ PLAYER TAG HIDDEN")
...
# ============================================================
# LUCAS SHOP - EXACT AD TEMPLATE V4
# ============================================================

def LUCAS_V4_GET(ad, *fields, default="ثبت نشده"):
    for f in fields:
        try:
            v = ad[f]
        except:
...
def LUCAS_V4_IMAGES(ad):
    imgs = []

    for f in ("image1", "image2", "image3"):
        try:
            x = ad[f]
        except:
            x = None
        ...
        if x:
            ...
            if x.startswith("http://") or x.startswith("https://"):
                ...
                imgs.append("/uploads/" + x)

    return imgs[:3]


def LUCAS_V4_PAGE(code):

    ad = LUCAS_V4_FIND(code)

    if ad is None:
        return redirect("/explore")

    try:
        number = int(ad["ad_number"])
    except:
        try:
            number = int(ad["id"])
        except:
    ...
    th = LUCAS_V4_NUM(ad, "townhall", "th")
    bh = LUCAS_V4_NUM(ad, "builderhall", "builder_hall", "bh")
    ...
    # تبدیل وضعیت‌ها
    if str(namechange).lower() in ("1", "true", "yes", "بله", "دارد"):
        namechange = "دارد"
    elif str(namechange).lower() in ("0", "false", "no", "خیر", "ندارد"):
        namechange = "ندارد"

    if str(supercell).lower() in ("1", "true", "yes", "بله", "دارد"):
        supercell = "متصل"
    elif str(supercell).lower() in ("0", "false", "no", "خیر", "ندارد"):
        supercell = "متصل نیست"

    imgs = LUCAS_V4_IMAGES(ad)
    ...
    for i, img in enumerate(imgs):
        thumbnails += f"""
        <div class="thumb {'active' if i == 0 else ''}"
             onclick="showPic('{img}',this)">
            <img src="{img}">
        </div>
        """

    # --------------------------------------------------------
    # قالب دقیق مشابه نمونه
    # --------------------------------------------------------
...
.template {{
    ...
    box-shadow:
        0 0 12px #087cff,
        inset 0 0 30px rgba(0,100,255,.16);
...
.photo {{
    ...
    box-shadow:
        inset 0 0 30px rgba(0,100,255,.18);
}}
...
<div class="template">
    ...
    <div class="panel">
        ...
        <div class="row">
            ...
            <div class="row-value hero-value">
                {king}
            </div>
...
{"<div class='description'><b>📝 توضیحات آگهی</b><br>" + description + "</div>" if description else ""}
...
# ============================================================
# جایگزینی route فعلی آگهی
# ============================================================

_lucas_v4_endpoint = None

for rule in app.url_map.iter_rules():

    if rule.rule == "/ad/<code>":

        _lucas_v4_endpoint = rule.endpoint
        break

if _lucas_v4_endpoint:
    app.view_functions[_lucas_v4_endpoint] = LUCAS_V4_PAGE
...
try:
    ...
    app.add_url_rule(
        "/ad/",
        endpoint="lucas_empty_ad_v4",
        view_func=lambda: redirect("/explore")
    )
...
print("======================================")
print("✅ LUCAS EXACT TEMPLATE V4 ACTIVE")
print("✅ ORIGINAL STRUCTURE")
print("✅ LARGE IMAGE LEFT")
print("✅ INFO PANEL RIGHT")
print("✅ AD CODE #1 #2 #3")
print("✅ TH / BH")
print("✅ ALL 4 HEROES")
print("✅ NAME CHANGE")
